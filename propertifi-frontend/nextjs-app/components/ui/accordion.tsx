...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn("border rounded-md shadow-sm", className)}
    {...props}
  />
))
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between p-4 text-lg font-semibold transition-all hover:bg-slate-100 [&[data-state=open]]:bg-slate-100 [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
